...
void main() async {
  ...
  while (pieceCount < 4000) {
    ...
    if (!fell) {
      pieceCount++;
      final hash =
          "${pit.relHeights.join(",")}|${pieceProducer.index % pieceProducer.list.length}|${gustProducer.index % gustProducer.list.length}";
      if (hashes.containsKey(hash)) {
        // print("cycle. current height: ${pit.tallest}, last height: ${hashes[hash]}, diff: ${pit.tallest-hashes[hash]}");
        print(
            "cycle. current piece: ${pieceCount}, last piece: ${hashes[hash]}, diff: ${pieceCount - hashes[hash]}");
      }
...
class Piece {
  ...
  Piece.withPos(
      this.filled, this.pos, this.width, this.height, this.topHeights);
  @override
  String toString() {
  ...
  void stickLikeGlue(Piece piece) {
    ...
    for (var i = 0; i < 7; i++) {
      if (piece.pos.x <= i && piece.width + piece.pos.x > i) {
        if (tallest + relHeights[i] <
            piece.topHeights[i - piece.pos.x] + piece.pos.y) {
          relHeights[i] =
              piece.topHeights[i - piece.pos.x] + piece.pos.y - tallest;
        }
      }
    }
